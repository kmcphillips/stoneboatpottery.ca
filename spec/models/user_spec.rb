...
describe User do
  ...
  describe "authenticate" do
    ...
    it "should find a user by name and password" do
      User.should_receive(:find).with(:conditions => ["username ? AND password = ?", "km", "pie_encrypted"]).and_return(@u)
      User.authenticate("km", "pie").should == @u
    end
    
    it "should not find a user because of invalid username or password" do
      User.should_receive(:find).with(:conditions => ["username ? AND password = ?", "km", "pie_encrypted"]).and_return(nil)
